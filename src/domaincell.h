...
struct domaincell_callbacks {
    int   (*try_lock)  (dcel *, const char * mode, void * lock_holder );
    int   (*unlock)   (dcel *, const char * mode, void * lock_holder );
};

struct hiena_domaincell {
	/** access by typcasting domainstream to (gob) */
	gob    general_object_chain_header;

	/** sync flags:  0 == out of sync, 1 == in sync */
	int    syncflags;			
	struct domaincell_callbacks * op;

	addr_t *addr;
	/* maybe these should move into addr? */
	void * serverlib;
	void * domain_ptr;
	void * protocol_addr;
